...
const orderInstructions = 
    ...
    "1️⃣ Додайте товари до кошика, вкажіть кількість продуктів\n" +
    "2️⃣ Перевірте правильність замовлення\n" +
    "3️⃣ Натисніть \"оформити замовлення\", залишіть контакти\n" +
    "4️⃣ Скопіюйте замовлення натиснувши \"Скопіювати\"\n" +
    "5️⃣ Відправте скопійоване замовлення у цей чат. Стрічка має починатися з 🌸 'Нове замовлення!'\n\n" +
    "✨ Успіх! Тут ваше замовлення точно зможе бути прочитане, а ваш подарунковий запит якомога швидше обробленим!";
...
bot.on('message', async (msg) => {
    try {
        if (msg.text === '/start') {
            ...
            await bot.sendMessage(msg.chat.id, 'Привіт! Це телеграм бот AVERY, який залюбки доставить твоє повідомлення до людини, що вирішить твоє подарункове питання 💐🍫🎁\n\nЗалиши своє повідомлення, і ми його неодмінно прочитаємо!', {
                reply_markup: keyboard
            });
        } else if (msg.text === '/store') {
            await bot.sendMessage(msg.chat.id, orderInstructions + "\n\n🛍️ Відвідайте наш магазин: avery.com.ua (для ПК)", {
                disable_web_page_preview: false
            });
        } else if (msg.text === '/shop') {
            const keyboard = {
                inline_keyboard: [
                    [{ text: '🛍️ Наш додаток в Telegram (для смартфона)', web_app: { url: 'https://avery.com.ua' } }]
                ]
            };
            await bot.sendMessage(msg.chat.id, orderInstructions, {
                reply_markup: keyboard
